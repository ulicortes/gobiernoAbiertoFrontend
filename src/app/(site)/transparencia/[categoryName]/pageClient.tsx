...
export default function DynamicCategoryClientPage({
  params,
}: {
  params: string;
}) {
  ...
  const categoryName = decodeURIComponent(params ?? "");
  ...
  useEffect(() => {
    const obtenerDatos = async () => {
      try {
        const resultado: any[] =
          await servicio.getArchivosDeUnaCategoria(categoryName);
        if (resultado) {
  ...
  const filesByYear = isEconomicReports
    ? rawFiles.reduce(
        (acc, file) => {
          const anio =
            file.year ||
            new Date(file.createdAt || file.date || new Date()).getFullYear();
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center mt-4 gap-4">
          {sortedYears.map((year) => (
            <ReporteEconomicoCard
              key={year}
              anio={year}
              archivos={filesByYear[year]}
            />
          ))}
        </div>
      ) : (
